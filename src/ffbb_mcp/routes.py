...
def register_routes(mcp: MCPServer) -> None:
    ...
    async def club_matches_api(request: Request) -> Response:
        ...
        from ffbb_mcp.services.http_api import fetch_club_matches

        result = await fetch_club_matches(
            organisme_id=organisme_id, team_filter=team_filter
        )
        status = (
            result.pop("status", 200)
            if isinstance(result, dict) and "status" in result
            else 200
        )
        # 404/500 déjà encodés par le service (error + matches vides)
