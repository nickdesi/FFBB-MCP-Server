...
def _build_dashboard_html() -> str:
    ...
    html = (
        ...
        "    header{background:var(--surface);border-bottom:2px solid var(--accent);padding:16px 28px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px}\n"
        "    .logo-area{display:flex;align-items:center;gap:12px}\n"
        "    .logo-area img{width:36px;height:36px;border-radius:6px;object-fit:contain}\n"
        "    .title{font-size:17px;font-weight:700;color:var(--accent);letter-spacing:.05em}\n"
        "    .meta{color:var(--muted);font-size:11px;margin-top:3px} .meta span{color:var(--text)}\n"
        ...
        "    .kpi{background:var(--surface);border:1px solid var(--border);border-radius:8px;padding:18px;transition:border-color .2s}\n"
        ...
        "    .bar-fill{height:100%;border-radius:3px;transition:width .4s ease} .bar-label{font-size:11px;margin-left:8px;font-variant-numeric:tabular-nums}\n"
        ...
        "    .ep-link{display:inline-flex;align-items:center;gap:6px;padding:7px 16px;border:1px solid var(--border);border-radius:6px;background:var(--surface);color:var(--text);text-decoration:none;font-size:12px;transition:all .15s}\n"
        "    .ep-link:hover{border-color:var(--accent);color:var(--accent);background:rgba(249,115,22,.07)}\n"
        "    .ep-link.active{border-color:var(--accent);color:var(--accent)}\n"
        ...
        "    .refresh-note{color:var(--muted);font-size:10px;display:flex;align-items:center;gap:6px}\n"
        "    .spin{display:inline-block;animation:spin 2s linear infinite;font-size:12px}\n"
        "    @keyframes spin{from{transform:rotate(0deg)}to{transform:rotate(360deg)}}\n"
        "  </style>\n"
        "</head>\n"
        "<body>\n"
        f"  <header>\n"
        f"    <div class='logo-area'>\n"
        f"      <img src='/logo.webp' alt='FFBB logo' onerror=\"this.style.display='none'\">\n"
        f"      <div>\n"
        f"        <div class='title'>&#127936; FFBB MCP SERVER</div>\n"
        f"        <div class='meta'>v<span>{_PACKAGE_VERSION}</span> &nbsp;&#183;&nbsp; transport <span>streamable-http</span> &nbsp;&#183;&nbsp; spec <span>2025-11-25</span></div>\n"
        f"      </div>\n"
        f"    </div>\n"
        f"    <div style='display:flex;align-items:center;gap:16px'>\n"
        f"      <span class='badge {status_badge_cls}'><span class='dot'></span>{status_label}</span>\n"
        f"      <span class='refresh-note'><span class='spin'>&#8635;</span> auto-refresh 10s</span>\n"
        f"    </div>\n"
        f"  </header>\n"
        "  <main>\n"
        ...
        f"    <span>Mise a jour : {now_utc} &nbsp;&#183;&nbsp; uptime {uptime_fmt} &nbsp;&#183;&nbsp; v{_PACKAGE_VERSION}</span>\n"
        f"  </footer>\n"
        "</body>\n"
        "</html>"
    )
