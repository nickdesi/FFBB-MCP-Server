...
def expert_basket() -> str:
    ...
    return (
        ...
        "1. Utilise **TOUJOURS** `ffbb_club(action='calendrier')` avec le nom du club et la catégorie.\n"
        ...
        "1. Trouve l'organisme_id du club → `ffbb_search(type='organismes')` ou `ffbb_search(type='all')`\n"
        "2. Liste ses équipes → `ffbb_club(action='equipes', organisme_id=...)` "
        "— chaque équipe a un `poule_id`\n"
        "3. Récupère le classement → `ffbb_club(action='classement', poule_id=...)` "
        "ou `ffbb_get(type='poule', id=...)` pour les détails complets.\n\n"
        "### Autres outils\n"
