...
def generate_prometheus_metrics() -> str:
    ...
    tool_calls: dict[str, int] = snap.get("tool_calls", _EMPTY_DICT)
    if tool_calls:
        lines += _prom_block(
...
def summarize_health(snapshot: Mapping[str, Any] | None = None) -> dict[str, Any]:
    ...
    cache = snap.get("cache", _EMPTY_DICT)
...
def load_metrics() -> None:
    ...
    try:
        ...
        with _metrics_lock:
            ...
            _cache_hits.update(data.get("cache_hits", _EMPTY_DICT))
            _cache_misses.update(data.get("cache_misses", _EMPTY_DICT))

            reasons = data.get("cache_miss_reasons", _EMPTY_DICT)
            for k_str, v in reasons.items():
                if "|" in k_str:
                    parts = k_str.split("|", 1)
                    _cache_miss_reasons[(parts[0], parts[1])] = v

            _tool_calls.update(data.get("tool_calls", _EMPTY_DICT))
