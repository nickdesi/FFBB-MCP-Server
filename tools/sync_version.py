...
def get_client_version(root_dir):
    ...
    match = re.search(r'\[\[package\]\]\s+name\s*=\s*"ffbb-api-client-v3"\s+version\s*=\s*"([^"]+)"', content, flags=re.DOTALL)
...
def update_website(root_dir, version):
    ...
    content = re.sub(r"(Dernière mise à jour du contenu\s*:\s*)[0-9]+\s+[\wÀ-ÿ]+\s+[0-9]+", f"\\g<1>{nice_date}", content)
    ...
    if sitemap_path.exists():
        ...
        s_content = re.sub(r"(<lastmod>)[^<]+(</lastmod>)", f"\\g<1>{now.strftime('%Y-%m-%d')}\\g<2>", s_content)
