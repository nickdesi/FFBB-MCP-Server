...
> Dernière mise à jour : FFBB MCP server | server.py: 2564 lignes | services.py: 10230 lignes
...
- `ffbb://rencontre/{rencontre_id}` → Détails d'une rencontre au format JSON
- `ffbb://salle/{salle_id}` → Détails d'une salle au format JSON
- `ffbb://officiel/{officiel_id}` → Détails d'un officiel/arbitre au format JSON
- `ffbb://entraineur/{entraineur_id}` → Détails d'un entraîneur/coach au format JSON
...
- **Machine à états stricte (Zéro résolution d'équipe ambiguë silencieuse)** : Ne JAMAIS renvoyer `status="resolved"` si plusieurs engagements actifs correspondent au club + catégorie (ex: Championnat [PLAT] vs Coupe [COUPE]). Renvoyer obligatoirement `status="ambiguous"`, la liste exhaustive `candidates`, et un `clarification_prompt` prêt pour le LLM. Propager systématiquement les filtres `competition_id`, `competition_type`, `poule_id`, `season_id`, `force_refresh` sur tous les outils d'équipe et sur `ffbb_club(action='calendrier')`.
- **Contrat de pagination `_meta` & enveloppe uniforme** : Respect strict de `1 <= limit <= 100` et `offset >= 0` sur `ffbb_search`. Tout outil retournant une collection paginée (`ffbb_search`, `ffbb_club(action='calendrier')`) doit renvoyer un objet `{"items": list, "_meta": dict}` avec `total`, `returned`, `has_more` et `sort`. Ne jamais insérer un objet sentinelle `_meta` en tête de liste.
- **Vérification d'identifiants numériques** : Toujours vérifier `id is None or id == ""` (ne jamais utiliser `if not id` qui élimine silencieusement `id: 0`).
- **Résolution H2H & Fallback de poule** : Si l'adversaire B n'est pas directement résolu par recherche globale d'organismes, inspecter la poule partagée de l'équipe A pour construire une résolution synthétique avec `fallback_warning`.
...
├── server.py              # Tools MCP + main() (≈2564 lignes)
├── services/              # Logique métier modularisée (≈10230 lignes)
...
- **Modularisation** : Le package `services/` (total ≈10230 lignes) remplace l'ancien fichier unique de 2915 lignes pour une meilleure cohésion.
