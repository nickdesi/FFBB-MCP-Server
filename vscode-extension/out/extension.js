...
async function openMcpInstallUri() {
    const opened = await vscode.env.openExternal(vscode.Uri.parse(MCP_INSTALL_URI));
    if (!opened) {
        await vscode.window.showErrorMessage('Impossible d’ouvrir le lien d’installation FFBB MCP. Utilisez la configuration .vscode/mcp.json du README.');
    }
}
function activate(context) {
    ...
    const hasShownWelcome = context.globalState.get(HAS_SHOWN_WELCOME_KEY, false);
    if (!hasShownWelcome) {
